...
express_append_toks = ['[E_CLS]', '[E_SEP]', '[E_MASK]', '[E_NON]']
...
class Alphabet(object):
    def __init__(
        ...
        if use_express_sorted:
            ...
            self.express_sorted_eos_idx = self.get_express_sorted_idx("[S_SEP]")
        else:
            self.express_sorted_list_filepath = None
        self.all_express_special_tokens = express_prepend_toks + express_append_toks
        self.all_express_special_token_ids = [self.get_express_idx(token) for token in self.all_express_special_tokens]
        if use_express_sorted:
