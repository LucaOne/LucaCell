...
# Parameters:
## 带默认值的参数 
* --lucaone_version: str, lucaone版本，默认使用lucaone版本, 预训练也是使用该版本，可以忽略使用默认值     
* --lucaone_step: str, lucaone的checkpoint，默认使用60000000, 预训练也是使用lucaone的该checkpoint，可以忽略使用默认值   
* --llm_dir: str, lucacell的checkpoint的存储路径，默认值../../ 即在该Project下，可以忽略使用默认值   
* --llm_type: str, 模型类型，默认值lucacell，可以忽略使用默认值     
* --llm_version: str, lucacell版本，这里使用lucacell-2048pos/v1.0，可以忽略使用默认值    
...
* --truncation_gene_seq_length: int, 样本中基因list的最多使用多少个基因，目前默认值4096，推理可以自由扩展，受限于计算资源，训练使用的长度是1200      
