...
class LucaPairIntraInter(BertPreTrainedModel, ABC):
    def __init__(self, config, args):
        super(LucaPairIntraInter, self).__init__(config)

        # gene_vs_matrix, matrix_vs_gene, matrix_vs_matrix
    ...
    def forward(
        ...
        if self.input_type == "matrix_vs_matrix":
            matrices_a = cell_encoded_matrices_a
            matrix_attention_masks_a = cell_matrix_attention_masks_a
            matrices_b = cell_encoded_matrices_b
            matrix_attention_masks_b = cell_matrix_attention_masks_b
        elif self.input_type == "gene_vs_matrix":
            matrices_a = gene_seqs_vector_input_a
            matrix_attention_masks_a = gene_seqs_vector_attention_masks_a
            matrices_b = cell_encoded_matrices_b
            matrix_attention_masks_b = cell_matrix_attention_masks_b
        elif self.input_type == "matrix_vs_gene":
            matrices_a = cell_encoded_matrices_a
            matrix_attention_masks_a = cell_matrix_attention_masks_a
            matrices_b = gene_seqs_vector_input_b
            matrix_attention_masks_b = gene_seqs_vector_attention_masks_b
        else:
        ...
        if output_classification_vector_dirpath and sample_ids:
            ...
            for sample_idx, sample_id in enumerate(sample_ids):
                ...
                output_classification_vectors.append(output_classification_vector)
        ...
        if output_classification_vector_dirpath and sample_ids:
            for sample_idx, sample_id in enumerate(sample_ids):
                output_classification_vectors[sample_idx].update({
                    "classification_vector": last_hidden_states[sample_idx].detach().cpu(),
                })
                filepath = os.path.join(output_classification_vector_dirpath, "%s_classification_vector.pt" % sample_id)
                torch.save(output_classification_vectors[sample_idx], filepath)
