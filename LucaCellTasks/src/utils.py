...
@file: utils
@desc: utils for LucaOnePlusTasks
'''
import math
import os, json
import subprocess
...
def to_device(device, batch):
    ...
    for item1 in batch.items():
        if isinstance(item1[1], dict):
            ...
            for item2 in item1[1].items():
                ...
                if isinstance(item2[1], dict):
                    for item3 in item2[1].items():
                        if item3[1] is None:
                            continue
                        elif item3[1] is not None and not isinstance(item3[1], int) and not isinstance(item3[1], str) and not isinstance(item3[1], float):
                    ...
                    if item2[1] is None:
                        continue
                    elif item2[1] is not None and not isinstance(item2[1], int) and not isinstance(item2[1], str) and not isinstance(item2[1], float):
            ...
            if item1[1] is None:
                continue
            elif item1[1] is not None and not isinstance(item1[1], int) and not isinstance(item1[1], str) and not isinstance(item1[1], float):
...
def seq_type_is_match_seq(seq_type, seq):
    ...
    if is_gene and seq_type in ["gene", "rna", "dna", "nucl"]:
        return True
...
def load_trained_logs_v1(log_filepath):
    args_info = None
            ...
            if v is not None:
                try:
                    ...
                    if debug_path is not None:
                        ...
                        if value.ndim == 3:
                            for dim_1_idx in range(value.shape[0]):
                                np.savetxt("%s_batch_%d.txt" % (key, dim_1_idx), value[dim_1_idx, :, :], fmt='%i', delimiter=",")
                        else:
...
def download_trained_checkpoint_lucacell(
    ...
    assert llm_type in ["lucacell", "lucacellv2"]
    assert llm_version in ["lucacell-2400/v1.0", "lucacell-2048pos/v1.0"]
    assert llm_task_name == "express_token_mask"
    assert llm_time_str is not None
    assert llm_step is not None
    try:
        logs_file_names = ["logs.txt"]
        if llm_version == "lucacell-2048pos/v1.0":
            models_file_names = ["config.json", "pytorch.pth", "training_args.bin", "tokenizer/alphabet.pkl", "tokenizer/express_bin_list.meta"]
        else:
            models_file_names = ["config.json", "pytorch.pth", "training_args.bin", "tokenizer/alphabet.pkl", "tokenizer/express_bin_list_2400.meta"]
