...
def complete_embedding_matrix(
    ...
    if init_emb is not None \
        ...
        ori_seq_len = len(sample_gene_express_bin_list)
        attn_list = []
        ...
        if lucacell_args.embedding_complete_seg_overlap:
            ...
            if lucacell_args.trunc_type == "right":
                ...
                for pos_idx in range(cur_segment_len, ori_seq_len - sliding_window, sliding_window):
                    ...
                    seg_gene_express_bin_list = sample_gene_express_bin_list[pos_idx - sliding_window:last_end]
                    print("segment idx: %d, seg express bin list len: %d" % (seg_idx, len(seg_gene_express_bin_list)))
                    seg_emb, seg_attn, seg_processed_nucleotide_seq_len, seg_processed_gene_seq_len = predict_embedding(
                        lucaone_args,
                        global_model_dirpath,
                        [sample_id + "_seg_%d" % seg_idx, sample_type, seg_gene_id_list, seg_gene_seq_list, seg_gene_express_bin_list],
                        embedding_type,
                        repr_layers=[-1],
                ...
                if last_end < ori_seq_len:
                    ...
                    seg_gene_express_bin_list = sample_gene_express_bin_list[ori_seq_len - 2 * sliding_window:ori_seq_len]
                    seg_emb, seg_attn, seg_processed_nucleotide_seq_len, seg_processed_gene_seq_len = predict_embedding(
                        lucaone_args,
                        global_model_dirpath,
                        [sample_id + "_seg_%d" % seg_idx, sample_type, seg_gene_id_list, seg_gene_seq_list, seg_gene_express_bin_list],
                ...
                for pos_idx in range(-cur_segment_len, -ori_seq_len + sliding_window, -sliding_window):
                    ...
                    seg_gene_express_bin_list = sample_gene_express_bin_list[last_start: pos_idx + sliding_window]
                    seg_emb, seg_attn, seg_processed_nucleotide_seq_len, seg_processed_gene_seq_len = predict_embedding(
                        lucaone_args,
                        global_model_dirpath,
                        [sample_id + "_seg_%d" % seg_idx, sample_type, seg_gene_id_list, seg_gene_seq_list, seg_gene_express_bin_list],
                ...
                if last_start > -ori_seq_len:
                    ...
                    seg_gene_express_bin_list = sample_gene_express_bin_list[-ori_seq_len:-ori_seq_len + 2 * sliding_window]
                    seg_emb, seg_attn, seg_processed_nucleotide_seq_len, seg_processed_gene_seq_len = predict_embedding(
                        lucaone_args,
                        global_model_dirpath,
                        [sample_id + "_seg_%d" % seg_idx, sample_type, seg_gene_id_list, seg_gene_seq_list, seg_gene_express_bin_list],
            ...
            for seg_idx in range(1, segment_num - 1):
                ...
                seg_gene_express_bin_list = sample_gene_express_bin_list[begin_seq_idx + seg_idx * cur_segment_len: begin_seq_idx + (seg_idx + 1) * cur_segment_len]
                seg_emb, seg_attn, seg_processed_nucleotide_seq_len, seg_processed_gene_seq_len = predict_embedding(
                    lucaone_args,
                    global_model_dirpath,
                    [sample_id + "_seg_%d" % seg_idx, sample_type, seg_gene_id_list, seg_gene_seq_list, seg_gene_express_bin_list],
            ...
            if lucacell_args.trunc_type == "right":
                ...
                last_seg_gene_express_bin_list = sample_gene_express_bin_list[-cur_segment_len:]
                ...
                last_seg_emb, last_seg_attn, last_seg_processed_nucleotide_seq_len, last_seg_processed_gene_seq_len = predict_embedding(
                    lucaone_args,
                    global_model_dirpath,
                    [sample_id + "_seg_%d" % (segment_num - 1), sample_type, last_seg_gene_id_list, last_seg_gene_seq_list, last_seg_gene_express_bin_list],
                    embedding_type,
                    repr_layers=[-1],
                    truncation_nucleotide_seq_length=truncation_nucleotide_seq_length,
                    truncation_gene_seq_length=truncation_gene_seq_length,
                ...
                first_seg_gene_express_bin_list = sample_gene_express_bin_list[:cur_segment_len]
                really_len = (ori_seq_len - (segment_num - 1) * cur_segment_len)
                first_seg_emb, first_seg_attn, first_seg_processed_nucleotide_seq_len, first_seg_processed_seq_len = predict_embedding(
                    lucaone_args,
                    global_model_dirpath,
                    [sample_id + "_seg_0", sample_type, first_seg_gene_id_list, first_seg_gene_seq_list, first_seg_gene_express_bin_list],
                    embedding_type,
                    repr_layers=[-1],
                    truncation_nucleotide_seq_length=truncation_nucleotide_seq_length,
                    truncation_gene_seq_length=truncation_gene_seq_length,
...
def encoder(
    ...
    assert len(sample_gene_id_list) == len(sample_gene_seq_list) == len(sample_gene_express_bin_list)
    gene_seq_emb_list = []
    if len(sample_gene_id_list) > truncation_gene_seq_length:
        if truncation_type_for_gene_seq == "right":
            ...
            sample_gene_express_bin_list = sample_gene_express_bin_list[:truncation_gene_seq_length]
        else:
            sample_gene_id_list = sample_gene_id_list[-truncation_gene_seq_length:]
            sample_gene_seq_list = sample_gene_seq_list[-truncation_gene_seq_length:]
            sample_gene_express_bin_list = sample_gene_express_bin_list[-truncation_gene_seq_length:]
    ...
    for gene_id, gene_seq in zip(sample_gene_id_list, sample_gene_seq_list):
        ...
        while seq_embed_vector is None:
            ...
            lucaone_args.seq = gene_seq
            seq_embed_matrix, cur_processed_nucleotide_seq_len, lucaone_model = predict_seq_embedding_main(lucaone_args)
    ...
    torch.cuda.empty_cache()
    express_encoded = tokenizer.express_encode(sample_gene_express_bin_list, add_special_token=True)
    return {
...
def predict_embedding(
    ...
    :param sample: [sample_id, sample_type, sample_gene_seq_list, sample_gene_express_bin_list]
    ...
    sample_id, sample_type, sample_gene_id_list, sample_gene_seq_list, sample_gene_express_bin_list = sample[0], sample[1], sample[2], sample[3], sample[4]
...
def get_args():
    ...
    parser.add_argument(
        "--sample_gene_express_bin_list_idx",
        type=int,
        default=None,
...
def main(lucacell_args):
    ...
    if lucacell_args.input_file and os.path.exists(lucacell_args.input_file):
        ...
        gene_seq_fasta = {}
        assert lucacell_args.sample_gene_id_list_idx and (lucacell_args.sample_gene_seq_list_idx is not None or lucacell_args.sample_gene_seq_meta_fasta is not None)
        if lucacell_args.sample_gene_seq_list_idx is None:
        ...
        for row in file_reader(lucacell_args.input_file):
            ...
            if gene_seq_from_fasta_flag:
                sample_id, sample_type, sample_gene_id_list, sample_gene_express_bin_list = \
                    row[lucacell_args.sample_id_idx].strip(), \
                    row[lucacell_args.sample_type_idx], \
                    eval(row[lucacell_args.sample_gene_id_list_idx]), \
                    eval(row[lucacell_args.sample_gene_express_bin_list_idx])
                sample_gene_seq_list = [gene_seq_fasta[gene_id] for gene_id in sample_gene_id_list]
            else:
                # 基因序列在输入文件中有
                sample_id, sample_type, sample_gene_id_list, sample_gene_seq_list, sample_gene_express_bin_list = \
                    row[lucacell_args.sample_id_idx].strip(), \
                    row[lucacell_args.sample_type_idx], \
                    eval(row[lucacell_args.sample_gene_id_list_idx]), \
            ...
            if not os.path.exists(embedding_filepath):
                input_gene_seq_list_len = len(sample_gene_seq_list)
                if lucacell_args.embedding_complete:
                    truncation_gene_seq_length = input_gene_seq_list_len
                else:
                    truncation_gene_seq_length = min(input_gene_seq_list_len, lucacell_args.truncation_gene_seq_length)
                while True:
                    # 设置了一次性推理长度
                    if lucacell_args.embedding_fixed_len_a_time_for_cell and lucacell_args.embedding_fixed_len_a_time_for_cell > 0:
                        ...
                        if emb is None:
                            ...
                            use_cpu = True
                        if emb is not None and input_gene_seq_list_len > lucacell_args.embedding_fixed_len_a_time_for_cell:
                        ...
                        if emb is None:
                            ...
                            use_cpu = True
                        # embedding完全长
                        if emb is not None and input_gene_seq_list_len > truncation_gene_seq_length:
                            emb, attns = complete_embedding_matrix(
                                lucaone_args,
                                lucacell_args,
                                sample_id,
    ...
    elif lucacell_args.sample_gene_id_list and lucacell_args.sample_gene_seq_list and lucacell_args.sample_gene_express_bin_list:
        print("Input gene express list length: %d" % len(lucacell_args.sample_gene_express_bin_list))
        ...
        lucacell_args.sample_gene_express_bin_list = lucacell_args.sample_gene_express_bin_list.upper().split(",")
        input_gene_seq_list_len = len(lucacell_args.sample_gene_express_bin_list)
        if lucacell_args.embedding_complete:
            truncation_gene_seq_length = input_gene_seq_list_len
        else:
            truncation_gene_seq_length = min(input_gene_seq_list_len, lucacell_args.truncation_gene_seq_length)
        while True:
            ...
            if lucacell_args.embedding_fixed_len_a_time_for_cell and lucacell_args.embedding_fixed_len_a_time_for_cell > 0:
                emb, attns, processed_nucleotide_seq_len, processed_gene_seq_len = predict_embedding(
                    lucaone_args,
                    global_model_dirpath,
                    [lucacell_args.sample_id, lucacell_args.sample_type, lucacell_args.sample_gene_id_list, lucacell_args.sample_gene_seq_list, lucacell_args.sample_gene_express_bin_list],
                    embedding_type="matrix",
                    repr_layers=[-1],
                    truncation_nucleotide_seq_length=lucacell_args.truncation_nucleotide_seq_length,
                    truncation_gene_seq_length=lucacell_args.embedding_fixed_len_a_time_for_cell,
                ...
                if emb is not None and input_gene_seq_list_len > lucacell_args.embedding_fixed_len_a_time_for_cell:
                    emb, attns = complete_embedding_matrix(
                        ...
                        lucacell_args.sample_gene_express_bin_list,
                        truncation_nucleotide_seq_length=lucacell_args.truncation_nucleotide_seq_length,
                        truncation_gene_seq_length=lucacell_args.embedding_fixed_len_a_time_for_cell,
                        init_emb=emb,
                        init_attns=attns,
        ...
        print("Express: %s" % lucacell_args.sample_gene_express_bin_list)
        print("Embedding: ")
        if embedding_type == "vector":
            emb = matrix_2_vector(emb, matrix_add_special_token, vector_type, lucacell_args.save_type)
        print(emb)
