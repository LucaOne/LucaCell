...
def create_pooler(
    ...
    if pooling_type == "max":
        ...
        return GlobalMaskSumPooling1D(axis=-2)
...
def create_output_loss_lucacell(
        task_level_type,
        task_level_name,
        config,
        args
    ...
    if not hasattr(args, "sigmoid"):
        ...
        args.sigmoid[task_level_type] = {}
    args.sigmoid[task_level_type][task_level_name] = False \
        if args.output_mode[task_level_type][task_level_name] \
           in ["multi_class", "multi-class", "regression"] \
        else True
    ...
    if task_level_type in ["gene_level", "whole_level"]:
...
def create_output_loss(
    ...
    dropout, hidden_layer, hidden_act, classifier, output, loss_fct = create_output_loss_lucacell(
        task_level_type,
        task_level_name,
        config,
        args
