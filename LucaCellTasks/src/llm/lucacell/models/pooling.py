...
class GlobalMaskAvgPooling1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        if mask is not None:
            ...
            mask = torch.unsqueeze(mask, dim=-1)
            x *= mask
            return torch.sum(x, dim=-2)/torch.sum(mask, dim=-2)
        else:
...
class GlobalMaskWeightedAttentionPooling1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = torch.sum(torch.unsqueeze(attention_probs, dim=-1) * x, dim=-2)
        return x
...
class GlobalMaskValueAttentionPooling1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            attention_probs = nn.Softmax(dim=-2)(e + torch.unsqueeze((1.0 - mask) * -10000, dim=-1))
        else:
            attention_probs = nn.Softmax(dim=-2)(e)
        x = torch.sum(attention_probs * x, dim=-2)
        del q, k, h, e, attention_probs
        return x
