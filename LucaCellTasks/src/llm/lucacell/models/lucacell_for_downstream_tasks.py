...
try:
    ...
    from common.loss import create_loss_function as create_downstream_tasks_loss_function
    from llm.lucacell.models.model_utils import AllOutput, create_output_loss_lucacell
    from llm.lucacell.models.alphabet import Alphabet
    from llm.lucacell.models.modeling_lucacell import *
    from llm.lucacell.models.pooling import *
    from llm.lucacell.models.configuration_lucacell import *
except ImportError:
    from src.common.pooling import *
    from src.common.loss import create_loss_function as create_downstream_tasks_loss_function
    from src.llm.lucacell.models.model_utils import AllOutput, create_output_loss_lucacell
    from src.llm.lucacell.models.alphabet import Alphabet
    from src.llm.lucacell.models.modeling_lucacell import *
    from src.llm.lucacell.models.pooling import *
    from src.llm.lucacell.models.configuration_lucacell import *


class LucaCellForDownstreamTasks(nn.Module):
    def __init__(
            self,
            lucacell_config: LucaCellConfig,
            lucacell_args,
        ...
        super().__init__()
        self.downstream_tasks_config = downstream_tasks_config
        self.downstream_tasks_args = downstream_tasks_args
        self.embed_dim = lucacell_config.embed_dim
        self.ffn_size = lucacell_config.ffn_size
        self.task_level_type = downstream_tasks_args.task_level_type

        self.nucleotide_input_type = lucacell_config.nucleotide_input_type
        if self.nucleotide_input_type in ["embedding_vector", "embedding_matrix"]:
            ...
            self.nucleotide_token_encoder_num_attention_heads = lucacell_config.nucleotide_token_encoder_num_attention_heads
            self.nucleotide_token_encoder_num_layers = lucacell_config.nucleotide_token_encoder_num_layers
        self.attention_heads = lucacell_config.num_attention_heads
        self.num_layers = lucacell_config.num_layers
        self.use_rotary_embeddings = lucacell_config.use_rotary_embeddings
        if self.use_rotary_embeddings:
            lucacell_config.no_nucleotide_position_embeddings = True
            lucacell_config.no_gene_positions_embeddings = True

        self.no_nucleotide_token_embeddings = lucacell_config.no_nucleotide_token_embeddings
        self.no_nucleotide_token_encoder = lucacell_config.no_nucleotide_token_encoder
        if self.no_nucleotide_token_encoder:
            self.nucleotide_token_encoder_num_attention_heads = 0
            self.nucleotide_token_encoder_num_layer = 0
        self.nucleotide_token_pooling_type = lucacell_config.nucleotide_token_pooling_type
        self.no_nucleotide_position_embeddings = lucacell_config.no_nucleotide_position_embeddings
        self.max_nucleotide_position_embeddings = lucacell_config.max_nucleotide_position_embeddings
        self.no_gene_positions_embeddings = lucacell_config.no_gene_positions_embeddings
        self.max_gene_position_embeddings = lucacell_config.max_gene_position_embeddings
        self.no_express_sorted_embeddings = lucacell_config.no_express_sorted_embeddings
        self.max_express_sorted_position_embeddings = lucacell_config.max_express_sorted_position_embeddings
        self.no_gene_type_embeddings = lucacell_config.no_gene_type_embeddings

        self.alphabet = Alphabet.from_predefined(
            lucacell_config.alphabet_type,
            gene_id_list_filepath=os.path.basename(lucacell_args["gene_id_list_filepath"]) if lucacell_args["gene_id_list_filepath"] else None,
            express_bin_list_filepath=os.path.basename(lucacell_args["express_bin_list_filepath"]),
            express_sorted_list_filepath=os.path.basename(lucacell_args["express_sorted_list_filepath"]) if lucacell_args["express_sorted_list_filepath"] else None,
            tokenizer_dir=os.path.join(downstream_tasks_args.model_dirpath, "tokenizer"),
            use_express_sorted="express_sorted_list_filepath" in lucacell_args and lucacell_args["express_sorted_list_filepath"] is not None
        )
        ...
        self.ignore_index = lucacell_config.ignore_index
        self.use_embed_layer_norm = lucacell_config.use_embed_layer_norm
        self.use_last_layer_norm = lucacell_config.use_last_layer_norm
        self.embed_scale = lucacell_config.embed_scale
        self.pretrained_model_name = lucacell_args["pretrained_model_name"]
        # 如果只用于embedding 推理则有些层不加载与构建
        if hasattr(downstream_tasks_args, "embedding_inference"):
            self.embedding_inference = downstream_tasks_args.embedding_inference
        else:
    ...
    def _init_submodules(self):
            ...
            if not self.no_nucleotide_token_embeddings:
                ...
                if not self.no_nucleotide_token_encoder:
                    ...
                    if self.use_last_layer_norm:
                        self.nucleotide_last_layer_norm = Luca1bLayerNorm(self.embed_dim)
                    else:
                        self.nucleotide_last_layer_norm = None
        ...
        if self.use_last_layer_norm:
            self.last_layer_norm = Luca1bLayerNorm(self.embed_dim)
        else:
        ...
        if not self.embedding_inference:
            self.dropout, self.hidden_layer, self.hidden_act, self.classifier, self.output, self.loss_fct = \
                create_downstream_tasks_loss_function(
                    self.downstream_tasks_config,
                    self.downstream_tasks_args,
                    hidden_size=self.embed_dim,
                    classifier_size=self.downstream_tasks_config.classifier_size,
                    sigmoid=self.downstream_tasks_args.sigmoid,
                    output_mode=self.downstream_tasks_args.output_mode,
                    num_labels=self.downstream_tasks_args.num_labels,
                    loss_type=self.downstream_tasks_args.loss_type,
                    ignore_index=self.downstream_tasks_args.ignore_index,
                    return_types=["dropout", "hidden_layer", "hidden_act", "classifier", "output", "loss"]
                )
            self.downstream_tasks_config.embedding_input_size = self.embed_dim
            if self.task_level_type in ["cell_level"]:
                self.matrix_pooler = create_pooler(
                    pooler_type="matrix",
                    config=self.downstream_tasks_config,
                    args=self.downstream_tasks_args
                )
    ...
    def _init_submodules_new(self, pretrained_model_name):
        ...
        try:
            ...
            from llm.lucaone.v2_0.alphabet import Alphabet
            from llm.lucaone.v2_0.lucaone_gplm_config import LucaGPLMConfig
            from llm.lucaone.v2_0.lucaone_gplm import LucaGPLM
        except ImportError:
            ...
            from src.llm.lucaone.v2_0.lucaone_gplm import LucaGPLM

        if self.num_layers <= 20:
            print("using LucaOne-Gene for weights init.")
            llm_type = "lucaone"
            llm_version = "lucaone-gene"
            llm_step = "36800000"
            llm_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
    ...
    def __forword__(
            self,
            nucleotide_input_ids: List[torch.Tensor] = None,
            nucleotide_position_ids: List[torch.Tensor] = None,
            nucleotide_input_embeds: List[torch.Tensor] = None,
            nucleotide_attention_mask: List[torch.Tensor] = None,
        ...
        if not self.embedding_inference and labels is not None:
            if self.matrix_pooler is not None:
                matrix_vector = self.matrix_pooler(x, mask=express_attention_mask)
            elif self.task_level_type in ["cell_level"]:
                tmp_mask = torch.unsqueeze(express_attention_mask, dim=-1)
                matrices = x.masked_fill(tmp_mask == 0, 0.0)
                # 均值pooling
                matrix_vector = torch.sum(matrices, dim=1)/(torch.sum(tmp_mask, dim=1) + 1e-12)
            else:
                matrix_vector = x
            if self.dropout is not None:
                matrix_vector = self.dropout(matrix_vector)
            if self.hidden_layer is not None:
                matrix_vector = self.hidden_layer(matrix_vector)
            if self.hidden_act is not None:
                matrix_vector = self.hidden_act(matrix_vector)
            logits = self.classifier(matrix_vector)
            ...
            if not return_simple:
                outputs = [representations, *outputs]
            else:
                outputs = [None, *outputs]
        else:
            outputs = [representations, None, None]
        return outputs
    ...
    def forward(
        ...
        representations, loss, logits = self.__forword__(
        ...
        if not self.embedding_inference:
            return loss, logits, representations
        else:
