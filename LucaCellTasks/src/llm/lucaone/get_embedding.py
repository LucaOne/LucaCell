...
try:
    ...
    from utils import set_seed, to_device, get_labels, get_parameter_number, load_trained_logs, \
        download_trained_checkpoint_lucaone_v2, clean_seq_luca, available_gpu_id, calc_emb_filename_by_sample_id, \
        matrix_2_vector, print_args, load_trained_model
    ...
    from src.utils import set_seed, to_device, get_labels, get_parameter_number, load_trained_logs, \
        download_trained_checkpoint_lucaone_v2, clean_seq_luca, available_gpu_id, calc_emb_filename_by_sample_id, \
        matrix_2_vector, print_args, load_trained_model
...
global_model_version, global_model_config, global_model, global_tokenizer = None, None, None, None, None, None, None
...
def load_model(
    ...
    if args_info["tokenization"]:
        ...
        tokenizer = AlphabetLucaOne.from_predefined("gene_prot")
    else:
...
def encoder(
    ...
    if args_info["tokenization"]:
        ...
        seqs = [seq]
        seq_types = [seq_type]
        seq_encoded_list = [tokenizer.encode(seq_type, seq)]
...
def get_embedding(
    ...
    if args_info["model_type"] in ["lucaone_gplm", "lucaone", "lucagplm"]:
        if seq_type == "gene":
            seq = tokenizer.gene_seq_replace(seq_type, seq)
        ...
        new_batch["return_contacts"] = False
    ...
    print("LucaOne embedding device:", device)
    model.to(device)
    model.eval()
    try:
        with torch.no_grad():
...
def predict_embedding(
    ...
    :param sample: [set_id, seq_type, seq]
    :param trunc_type:
    :param embedding_type: bos or representations
    :param repr_layers: [-1]
    :param truncation_seq_length: [4096, 2048, 1984, 1792, 1536, 1280, 1152, 1024]
    :param device:
    ...
    if isinstance(llm_dirpath, str):
        cur_log_filepath = os.path.join(
            os.path.dirname(llm_dirpath).replace("models", "logs"),
            "logs.txt"
        )
    ...
    if isinstance(llm_dirpath, str):
        ...
        if seq_type in ["gene", "dna", "rna", "nucl"]:
            emb, processed_seq_len = get_embedding(
    ...
    if "representations" in embedding_type or "matrix" in embedding_type:
    ...
    elif "bos" in embedding_type or "vector" in embedding_type or "cls" in embedding_type:
        if seq_type == "prot":
            embedding = emb.hidden_states_b
        else:
            embedding = emb.hidden_states
        ...
        if seq_type == "prot":
            ...
            embedding = emb.hidden_states
        if save_type == "numpy":
            embeddings["mean_representations"] = torch.mean(embedding[0, 1:-1, :], dim=0).to(device="cpu").clone().numpy()
        else:
            embeddings["mean_representations"] = torch.mean(embedding[0, 1:-1, :], dim=0).to(device="cpu").clone()
...
def complete_embedding_matrix(
    ...
    if init_emb is not None \
            and model_args.embedding_complete \
...
def embedding_for_one_sample(model_args):
    global global_log_filepath, global_model_dirpath, global_args_info, \
    ...
    if model_args.seq and model_args.seq_type:
        ...
        return emb, processed_seq_len, global_model
    else:
...
def main(model_args):
    global global_log_filepath, global_model_dirpath, global_args_info, \
        global_model_config, global_model, global_tokenizer
    print("*" * 20 + "LucaOne Model Args:" + "*" * 20)
    ...
    print("Model args device: %s" % model_args.device)
    embedding_type = model_args.embedding_type
    vector_type = model_args.vector_type
    if embedding_type == "vector" and vector_type == "cls":
        matrix_add_special_token = True
    ...
    if model_args.input_file and os.path.exists(model_args.input_file):
        ...
        for row in file_reader(model_args.input_file):
            ...
            emb_filename = calc_emb_filename_by_sample_id(sample_id=seq_id, embedding_type=embedding_type)
