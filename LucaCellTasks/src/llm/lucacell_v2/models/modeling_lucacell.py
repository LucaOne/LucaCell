...
class Luca1LayerNorm(nn.Module):
...
try:
    ...
    class Luca1bLayerNorm(_FusedLayerNorm):
        @torch.jit.unused
        def forward(self, x):
            if not x.is_cuda:
    ...
    from torch.nn import LayerNorm as Luca1bLayerNorm


class LucaTransformerLayer(nn.Module):
    """Luca Transformer layer block."""
    ...
    def __init__(
        ...
        :param use_luca1b_layer_norm:  whether to use luca 1b layer norm
        ...
        self._init_submodules(add_bias_kv, use_luca1b_layer_norm)

    def _init_submodules(self, add_bias_kv, use_luca1b_layer_norm):
        LucaLayerNorm = Luca1bLayerNorm if use_luca1b_layer_norm else Luca1LayerNorm

        # pre layer norm
        self.pre_layer_norm = LucaLayerNorm(self.embed_dim, eps=1e-8)
        ...
        # post layer norm
        self.post_layer_norm = LucaLayerNorm(self.embed_dim, eps=1e-8)
...
class LucaAxialTransformerLayer(nn.Module):
    def __init__(
            self,
        ...
        row_self_attention = LucaRowSelfAttention(
            embedding_dim,
            num_attention_heads,
        ...
        column_self_attention = LucaColumnSelfAttention(
            embedding_dim,
            num_attention_heads,
            dropout=attention_dropout,
            max_tokens_per_msa=max_tokens_per_msa,
        ...
        feed_forward_layer = LucaFeedForwardNetwork(
            embedding_dim,
            ffn_embedding_dim,
    ...
    def build_residual(self, layer: nn.Module):
        return LucaNormalizedResidualBlock(
            layer,
            self.embedding_dim,
            self.dropout_prob,
        )
...
class LucaLearnedPositionalEmbedding(nn.Embedding):
    def __init__(
            self,
            num_embeddings: int,
            embedding_dim: int,
...
class LucaSinusoidalPositionalEmbedding(nn.Module):
...
class LucaRobertaLMHead(nn.Module):
    def __init__(self, embed_dim, output_dim, weight):
        super().__init__()
        self.dense = nn.Linear(embed_dim, embed_dim)
        self.layer_norm = Luca1bLayerNorm(embed_dim, eps=1e-8)
        self.weight = weight
...
class LucaNormalizedResidualBlock(nn.Module):
    def __init__(
            self,
            layer: nn.Module,
            embedding_dim: int,
        ...
        self.layer_norm = Luca1bLayerNorm(self.embedding_dim)
...
class LucaFeedForwardNetwork(nn.Module):
    def __init__(
            self,
            embedding_dim: int,
...
class LucaRowSelfAttention(nn.Module):
...
class LucaColumnSelfAttention(nn.Module):
...
class LucaFairseqIncrementalState(object):
...
def with_incremental_state(cls):
    cls.__bases__ = (LucaFairseqIncrementalState,) + tuple(
        b for b in cls.__bases__ if b != LucaFairseqIncrementalState
    )
    return cls


@with_incremental_state
class LucaMultiheadAttentionV1(nn.Module):
    def __init__(
            self,
            embed_dim,
            num_heads,
        ...
        if use_rotary_embeddings:
            self.rot_emb = LucaRotaryEmbedding(dim=self.head_dim)
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = LucaMultiheadAttention._append_prev_key_padding_mask(
        ...
        attn_weights = LucaMultiheadAttention.apply_sparse_mask(
...
class LucaRotaryEmbedding(torch.nn.Module):
