...
@file: file_operator
@desc: file operator for LucaOnePlusTasks
'''
import csv, sys
...
def clean_seq_esm(protein_id, seq):
    seq = seq.upper()
    new_seq = ""
...
def csv_writer(dataset, handle, header, append=False):
    '''
    csv 写，适合大文件
    :param dataset: 数据
    :param handle: 文件
    ...
    :return:
    '''
    handle = handle if isinstance(handle, io.TextIOWrapper) else open(handle, 'a+' if append else 'w')
    try:
        ...
        with open(filepath, "w") as output_handle:
            if len(sequences[0]) > 1 and isinstance(sequences[0][0], str):
                for row in sequences:
                    ...
                    sequence = SeqRecord(
                        Seq(seq, None),
                        id=seq_id[1:] if seq_id and seq_id[0] == ">" else seq_id,
                        description=""
                    )
