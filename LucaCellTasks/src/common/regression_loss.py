...
def iqr(batch, dim=None, reduction='mean'):
    ...
    if isinstance(batch, np.ndarray):
        ...
        raise Exception("Not support the dtype of input.")
    if reduction == 'none':
        return out
    elif reduction == 'mean':
        return out.mean()
...
class DynamicWeightedMSE(nn.Module):
    def __init__(self, high_weight=10.0, low_weight=1.0, threshold=0.5):
    ...
    def forward(self, pred, target):
        # pred: [batch_size, 2000]
        # target: [batch_size, 2000] (这是真实的 delta 值)

        # 1. 计算每个元素的平方误差
        mse_loss = (pred - target) ** 2

        # 2. 根据 target 动态生成权重矩阵
        # 如果真实变化的绝对值大于阈值，给高权重，否则给低权重
        weights = torch.where(
            torch.abs(target) > self.threshold,
            torch.ones_like(target) * self.high_weight,
            torch.ones_like(target) * self.low_weight
        )

        # 3. 特殊加权：给“扰动基因”本身额外的权重 (可选)
        # 如果你知道哪个维度是那个被扰动的基因，可以再给它加 100 倍权重
