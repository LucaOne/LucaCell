...
class LucaTransformerLayer(nn.Module):
    ...
    def _init_submodules(self):
        if self.layer_norm_type == "pre":
            # pre layer norm
            self.pre_layer_norm = nn.LayerNorm(self.embed_dim)
...
class LucaTransformer(nn.Module):
    def __init__(self, config, emb_layer=None, use_pretrained_embedding=False, add_pooling_layer=True):
        ...
        if self.layer_norm_type == "pre":
            self.last_layer_norm = nn.LayerNorm(self.config.hidden_size)
    ...
    def forward(
            ...
            gene_express_input_ids: Optional[torch.Tensor] = None,
            attention_mask: Optional[torch.Tensor] = None,
            token_type_ids: Optional[torch.Tensor] = None,
            position_ids: Optional[torch.Tensor] = None,
            inputs_embeds: Optional[torch.Tensor] = None,
            gene_seq_inputs_embeds: Optional[torch.Tensor] = None,
            return_attentions=False
    ):
        if self.use_pretrained_embedding:
            x = self.embeddings(inputs_embeds)
        elif gene_seq_inputs_embeds is not None and gene_express_input_ids is not None:
            x = gene_seq_inputs_embeds + self.embeddings(
                input_ids=gene_express_input_ids,
        ...
        if attention_mask is None:
            if input_ids is not None:
                ...
                padding_mask = gene_express_input_ids.eq(self.padding_idx)
        else:
...
class LucaCrossTransformer(nn.Module):
    def __init__(
        ...
        if self.layer_norm_type == "pre":
            ...
            self.embed_layer_norm_a, self.embed_layer_norm_b = nn.LayerNorm(self.embed_dim), nn.LayerNorm(self.embed_dim)
        ...
        self.encoder_a = nn.ModuleList(
            [
                LucaCrossTransformerLayer(
                    ...
                    layer_norm_type=config.layer_norm_type,
                    attention_type="cross",
                    use_rotary_embeddings=config.use_rotary_position_embeddings_for_cross,
                    )
                for _ in range(self.config.num_hidden_layers)
            ]
        )
        ...
        self.layer_size = self.config.num_hidden_layers
        if self.layer_norm_type == "pre":
            self.last_layer_norm_a = nn.LayerNorm(self.config.hidden_size)
            if self.shared:
                self.last_layer_norm_b = self.last_layer_norm_a
            else:
                self.last_layer_norm_b = nn.LayerNorm(self.config.hidden_size)
...
class LucaEmbeddings(nn.Module):
    ...
    def __init__(self, config, init_weight_filepath=None):
        super().__init__()
        if hasattr(config, "no_token_embeddings"):
            self.no_token_embeddings = config.no_token_embeddings
        else:
            self.no_token_embeddings = False
        if hasattr(config, "use_embed_layer_norm"):
            self.use_embed_layer_norm = config.use_embed_layer_norm
        else:
            self.use_embed_layer_norm = False
        if not self.no_token_embeddings:
            self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)
            if init_weight_filepath:
                loaded_weights_tensor = torch.load(init_weight_filepath)
                self.word_embeddings.weight.data.copy_(loaded_weights_tensor)
                print("load the predefined embedding layer weights from %s." % init_weight_filepath)
