...
atom_prepend_toks = ['[ATOM_PAD]', '[ATOM_UNK]', '[ATOM_CLS]']

atom_append_toks = ['[ATOM_SEP]', '[ATOM_MASK]']


class AlphabetAtom(object):
    def __init__(
            self,
            standard_toks: List[str] = atom_standard_toks,
            prepend_toks: List[str] = atom_prepend_toks,
            append_toks: List[str] = atom_append_toks,
        ...
        self.unk_idx = self.tok_to_idx["[ATOM_UNK]"]
        self.padding_idx = self.get_idx("[ATOM_PAD]")
        self.pad_idx = self.get_idx("[ATOM_PAD]")
        self.pad_token_id = self.padding_idx
        self.cls_idx = self.get_idx("[ATOM_CLS]")
        self.mask_idx = self.get_idx("[ATOM_MASK]")
        self.eos_idx = self.get_idx("[ATOM_SEP]")
