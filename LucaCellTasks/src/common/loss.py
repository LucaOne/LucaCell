    ...
    if "loss" in return_types:
        ...
        if output_mode in ["regression"]:
            if loss_type == "l2":
                ...
                loss_fct = MaskedL1Loss(
                    reduction=reduction,
                    ignore_nans=True,
                    ignore_value=ignore_index * 1.0 if ignore_index else None
                )
            elif loss_type == "dynamic_weighted_mse":
                print("Using DynamicWeightedMSE: threshold: %f, low_weight: %f, high_weight: %f" % (
                    args.dynamic_weighted_mse_threshold,
                    args.dynamic_weighted_mse_low_weight,
                    args.dynamic_weighted_mse_high_weight
                ))
                loss_fct = DynamicWeightedMSE(
                   threshold=args.dynamic_weighted_mse_threshold,
                    low_weight=args.dynamic_weighted_mse_low_weight,
                    high_weight=args.dynamic_weighted_mse_high_weight
                )
            else:
            ...
            if loss_type == "bce":
                ...
                loss_fct = MaskedAsymmetricLossOptimized(
                ...
                loss_fct = MaskedFocalLossBinaryClass(
                    alpha=args.focal_loss_alpha if hasattr(args, "focal_loss_alpha") else 0.7,
            ...
            if loss_type == "bce":
                if pos_weight:
                    ...
                    if isinstance(pos_weight, str) or isinstance(pos_weight, int):
                        ...
                        pos_weight = torch.tensor([pos_weight], dtype=torch.float32).to(args.device)
                    print("task_type='binary_class', loss pos_weight:")
                    print(pos_weight)
            ...
            if loss_type == "focal_loss":
                if hasattr(args, "focal_loss_alpha") and args.focal_loss_alpha:
                    ...
                    print("task_type='multi_class', loss alpha:")
                    print(alpha)
                else:
                    alpha = torch.tensor([1.0] * num_labels, dtype=torch.float32).to(args.device)
                gamma = args.focal_loss_gamma if hasattr(args, "focal_loss_gamma") else 2.0
                loss_fct = MaskedFocalLossMultiClass(
                    alpha=alpha,
                    gamma=gamma,
                    normalization=True,
                    reduction=reduction,
                ...
                if weight:
                    ...
                    print("task_type='multi_class', loss weight:")
                    print(weight)
