...
def load_model(args, model_dir):
    ...
    if args.model_type == "lucapair_homo":
        config_class, seq_tokenizer_class, cell_tokenizer_class, model_class \
            = LucaConfig, AlphabetLucaOne, AlphabetLucaCell, LucaPairHomo
    elif args.model_type == "lucapair_heter":
        config_class, seq_tokenizer_class, cell_tokenizer_class, model_class \
            = LucaConfig, AlphabetLucaOne, AlphabetLucaCell, LucaPairHeter
    elif args.model_type == "lucapair_intrainter":
        config_class, seq_tokenizer_class, cell_tokenizer_class, model_class \
            = LucaConfig, AlphabetLucaOne, AlphabetLucaCell, LucaPairIntraInter
    elif args.model_type == "lucasingle":
        config_class, seq_tokenizer_class, cell_tokenizer_class, model_class \
            = LucaConfig, AlphabetLucaOne, AlphabetLucaCell, LucaSingle
    else:
    ...
    model_config.num_labels = model_args.model_config
...
def run(
    ...
    model_args.seq_vector_dirpath = model_args.seq_emb_dir if model_args.seq_emb_dir and (os.path.exists(model_args.seq_emb_dir) or "#" in model_args.seq_emb_dir) else None
    model_args.seq_matrix_dirpath = model_args.seq_emb_dir if model_args.seq_emb_dir and (os.path.exists(model_args.seq_emb_dir) or "#" in model_args.seq_emb_dir) else None
    ...
    model_args.cell_vector_dirpath = model_args.cell_emb_dir if model_args.cell_emb_dir and (os.path.exists(model_args.cell_emb_dir) or "#" in model_args.cell_emb_dir) else None
    model_args.cell_matrix_dirpath = model_args.cell_emb_dir if model_args.cell_emb_dir and (os.path.exists(model_args.cell_emb_dir) or "#" in model_args.cell_emb_dir) else None
...
if __name__ == "__main__":
    ...
    seq_fasta = {}
    if args.seq_fasta_filepath:
        if not os.path.exists(args.seq_fasta_filepath):
            sys.exit("%s not exists" % args.seq_fasta_filepath)
        for row in fasta_reader(args.seq_fasta_filepath):
            seq_id = row[0]
            if seq_id[0] == ">":
                seq_id = seq_id[1:]
    ...
    if args.input_file is not None and os.path.exists(args.input_file):
        ...
        data = [[args.sample_id, args.sample_type, args.sample_gene_id_list, args.sample_gene_seq_type_list, args.sample_gene_seq_list, args.sample_gene_express_bin_list]]
        results = run(
            samples=data,
            seq_max_length=args.seq_max_length,
            seq_emb_dir=args.seq_emb_dir,
