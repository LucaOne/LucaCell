    ...
    def read_one_line(self):
        try:
            ...
            if self.input_mode == "pair":
                if len(row) == 13:
                    sample_id_a, sample_id_b, sample_type_a, sample_type_b, sample_gene_id_list_a, sample_gene_id_list_b, \
                        sample_gene_seq_type_list_a, sample_gene_seq_type_list_b, sample_gene_seq_list_a, sample_gene_seq_list_b, \
                        sample_gene_express_bin_list_a, sample_gene_express_bin_list_b, label = row[0:13]
                else:
                    raise Exception("the cols num not equals 13")
                res = {
                    "sample_id_a": sample_id_a,
                    "sample_id_b": sample_id_b,
                    "sample_type_a": sample_type_a,
                    "sample_type_b": sample_type_b,
                    "sample_gene_id_list_a": eval(sample_gene_id_list_a) if sample_gene_id_list_a else None,
                    "sample_gene_id_list_b": eval(sample_gene_id_list_b) if sample_gene_id_list_b else None,
                    "sample_gene_seq_type_list_a": eval(sample_gene_seq_type_list_a) if sample_gene_seq_type_list_a and "[" in sample_gene_seq_type_list_a else (sample_gene_seq_type_list_a if sample_gene_seq_type_list_a else None),
                    "sample_gene_seq_type_list_b": eval(sample_gene_seq_type_list_b) if sample_gene_seq_type_list_b and "[" in sample_gene_seq_type_list_b else (sample_gene_seq_type_list_b if sample_gene_seq_type_list_b else None),
                    "sample_gene_seq_list_a": eval(sample_gene_seq_list_a) if sample_gene_seq_list_a else None,
                    "sample_gene_seq_list_b": eval(sample_gene_seq_list_b) if sample_gene_seq_list_b else None,
                    "sample_gene_express_bin_list_a": eval(sample_gene_express_bin_list_a) if sample_gene_express_bin_list_a else None,
                    "sample_gene_express_bin_list_b": eval(sample_gene_express_bin_list_b) if sample_gene_express_bin_list_b else None,
                }
                ...
                if len(row) == 7:
                    sample_id, sample_type, sample_gene_id_list, sample_gene_seq_type_list, \
                        sample_gene_seq_list, sample_gene_express_bin_list, label = row[0:7]
                else:
                    raise Exception("the cols num not equals 7")
                res = {
                    "sample_id": sample_id,
                    "sample_type": sample_type,
                    "sample_gene_id_list": eval(sample_gene_id_list) if sample_gene_id_list else None,
                    "sample_gene_seq_type_list": eval(sample_gene_seq_type_list) if sample_gene_seq_type_list and "[" in sample_gene_seq_type_list else (sample_gene_seq_type_list if sample_gene_seq_type_list else None),
                    "sample_gene_seq_list": eval(sample_gene_seq_list) if sample_gene_seq_list else None,
                    "sample_gene_express_bin_list": eval(sample_gene_express_bin_list) if sample_gene_express_bin_list else None
                }
                if not self.inference:
                    res.update({
                        "label": label
